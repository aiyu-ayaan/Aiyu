...
## TeX Live source (revised — risk #1 materialized)

During implementation, SwiftLaTeX's package CDN (`texlive2.swiftlatex.com`) was
found permanently dead, and no public mirror or committed static cache exists.
The engine fetches its core `.fmt` *and every package* from that server, so
nothing compiles without a replacement source. Resolution (chosen: vendor a
static cache):

- The TeX files are **self-hosted** under `public/texlive-cache` and served by a
  `/texlive/pdftex/[...seg]` Node route. The engine writes each downloaded file
  to a path keyed by a `fileid` response header, so a plain static host fails —
  the route attaches that header (and returns 301 for misses, which the worker
  treats as "not found"). Read-only-container safe (reads committed files only).
- `swiftlatexEngine` points `setTexliveEndpoint` at our own `/texlive/` origin.
  The vendored `PdfTeXEngine.js` is patched: `setTexliveEndpoint` no longer nulls
  the worker (an upstream bug that broke every subsequent call).
- The cache is generated by a **one-time Docker bootstrap**,
  `npm run resume:texlive` (`scripts/build-texlive-cache.mjs`): compiles the
  default template with `pdflatex -recorder` in `texlive/texlive`, vendors the
  touched files (flat, by basename) + SwiftLaTeX's `.fmt`, commits them.
- `next.config` traces `public/texlive-cache` into the standalone build.
- Adding packages: if a compile logs `TexLive File not exists <x>`, add the
  package and re-run the bootstrap.

## Risks

1. **One-time bootstrap needs Docker/TeX** — the cache must be generated once on a
   machine with Docker (the deploy host already has it). Couldn't be run in the
   build sandbox; iterate on first run.
...
4. **Package coverage** — the recorder captures the default template's files; a
   résumé using new packages needs a cache refresh.
